#!/usr/bin/env python3
"""TabNet and FT-Transformer on bio-only features (d=482), 15 tasks, 5 seeds."""
import os, sys, json, time
import numpy as np
import pandas as pd
import torch
sys.path.insert(0, "/home/zihend1/Genesis/TargetSage2")
...
# Load data
print("[DATA] loading bio-only features...")
...
class FTTransformer(nn.Module):
    def __init__(self, d_in, d_emb=64, n_heads=8, n_layers=3):
        super().__init__()
        ...
        layer = nn.TransformerEncoderLayer(d_emb, n_heads, d_emb*2, dropout=0.1, norm_first=True, batch_first=True)
...
class ResNetMLP(nn.Module):
    def __init__(self, d_in, d_h=256, n_blocks=4):
        super().__init__()
        self.proj = nn.Linear(d_in, d_h)
        self.blocks = nn.ModuleList([nn.Sequential(nn.LayerNorm(d_h), nn.Linear(d_h, d_h*2), nn.ReLU(), nn.Dropout(0.1), nn.Linear(d_h*2, d_h)) for _ in range(n_blocks)])
...
def train_torch(model, X_tr, y_tr, X_te, y_te, epochs=80, lr=1e-3, bs=512):
...
for task in TASKS:
    ...
    print(f"\n[{DISPLAY[task]}] |P|={n_pos}, n_seeds={n_seeds}")
    for method in ["FT-Trans", "ResNet"]:
        scores = []
        for seed in range(n_seeds):
            ...
            if method == "FT-Trans":
                model = FTTransformer(X_tr.shape[1]).to(device)
            else:
                model = ResNetMLP(X_tr.shape[1]).to(device)
            af1 = train_torch(model, X_tr, y_tr, X_te, y_te, epochs=80)
            scores.append(af1)
        m, s = np.mean(scores), np.std(scores)
        print(f"  {method}: {m:.2f} ± {s:.2f}")
        results.append({"task": task, "display": DISPLAY[task], "method": method, "mean": m, "std": s, "n_seeds": n_seeds})
        pd.DataFrame(results).to_csv("results/dl_bio_only.csv", index=False)

print("\nDONE - results in results/dl_bio_only.csv")
